...
| `probe-client.test.mjs` | Separate session/body requests, sequential transfer and lost ACK/port behavior |
...
| `probe-receiver.test.mjs` | Private one-probe staging, identity/fences, durable receipts and fail-closed restart |
| `probe-integration.test.mjs` | Actual collector function through native framing and a real temporary Unix socket to SQLite staging; exact digest and secret sentinels |
